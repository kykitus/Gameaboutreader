...
void bar_writer(enemy* opponent, int round) {
	...
	std::cout << "\n\nHP: ";
	for (int i = 1; i <= opponent->gethp(); i++) {
		std::cout << "/";
	...
	std::cout << "\n\n\n\n";
	std::cout << "YOU\nHP: ";
	for (int i = 1; i <= gameplayer.gethp(); i++) {
		std::cout << "/";
		i++;
		if (i % 100 == 0) {
			std::cout << "\n";
	...
	std::cout << "\nSTAMINA: ";
	for (int i = 1; i <= gameplayer.gettiredness(); i++) {
		std::cout << "/";
		if (i % 100 == 0) {
			std::cout << "\n";
		}
...
bool attackprocess(livingentity* user, livingentity* victim, int attacktype) {
	...
	std::string enter;
	int attackseed = randomnumber(100);
	int atk = user->getatk();
	...
	float bonus = 1;
	if (user->getdistance() > user->getrange() + userwep->getrange() || attackseed > user->getaccuracy()) {
