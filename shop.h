...
void weapon_shop() {
	...
	while (true) {
		...
		try {
			...
			std::cin >> enter;
			if (enter == "y" || enter == "Y") {
				if (gameplayer.getidofwep() == (std::to_string(categoryofweapon) + choose)) {
...
void armor_shop() {
	...
	while (true) {
		...
		try {
			...
			std::cin >> enter;
			if (enter == "y" || enter == "Y") {
				if (gameplayer.getidofarmor() == (std::to_string(categoryofarmor) + choose) || gameplayer.getidofhelm() == (std::to_string(categoryofarmor) + choose)) {
					std::cout << "you already have it";
					enter = _getch();
					continue;
				}
