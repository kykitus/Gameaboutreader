...
int armordisplay(std::vector<armor*> aror, int u) {
	...
	while (g < aror.size()) {
		...
		std::cout << "-[" << u << "]- " << aror[g]->getname() << "\t| hardness - " << aror[g]->gethardness() << "m\t||| price: ";
	...
	while (true) {
		...
		try {
			...
			if (agreement == "y" || agreement == "Y") {
				...
				if (playere.getidofarmor() == chose) {
					std::cout << "you already have it";
					enter = _getch();
					continue;
				}
				else if (playere.getidofhelm() == chose) {
					std::cout << "you already have it";
					enter = _getch();
					continue;
				}
