...
extern std::string typetranslator(unsigned int category, unsigned int typeofequipment);


class item { // main equipable class
protected:

	std::string name = "example_item";
	unsigned int price = 1;
	unsigned int lvlreq = 1;
	bool isbought = false;
...
public:
	...
	int getprice();
	void setprice(unsigned int val);
	int getlvlreq();
	void setlvlreq(unsigned int val);
	bool getisbought();
	void setisbought(bool val);
...
private:
	...
	unsigned int valueattack; //first one for shop
	unsigned int sideattack;
	unsigned int sideattacktiring;
	unsigned int aboveattack;
	unsigned int aboveattacktiring;
	unsigned int stabattack;
	unsigned int stabattacktiring;
	float range;

	unsigned int classweapon;

public:

	int getvalueattack();
	void setvalueattack(unsigned int val);
	int getdirectionattack(unsigned int attacktype);
	int getdirectiontiring(unsigned int attacktype);
	void setdirectionattack(unsigned int val, unsigned int weapontype);
	float getrange();
	void setrange(float val);
	unsigned int getclassweapon();
	void setclassweapon(unsigned int val);

	weapon() = default;
	weapon(std::string wname, unsigned int wattack, float wrange, unsigned int wprice, unsigned int wlvlreq, unsigned int wclassweapon) {
		name = wname;
...
private:
	...
	unsigned int isforhead;
	float hardness;
	unsigned int classarmor;

public:

	float gethardness();
	void sethardness(float val);
	bool getisforhead();
	...
	armor() = default;
	armor(std::string aname, float ahardness, unsigned int aprice, unsigned int alvlreq, unsigned int aisforhead, unsigned int aclassarmor) {
		name = aname;
		hardness = ahardness;
		price = aprice;
		lvlreq = alvlreq;
...
public:
	...
	luckyitem() = default;
	luckyitem(std::string iname, unsigned int iprice, unsigned int ilvlreq, std::string icategory, bool havepassive, std::string iabilitydes) {
		name = iname;
		price = iprice;
		lvlreq = ilvlreq;
		category = icategory;
...
private:
	...
	unsigned int tirednessfixpower;
	unsigned int attack;
	unsigned int tier;
...
public:
	...
	int gethealpower();
	void sethealpower(unsigned int val);
	float getresistancepower();
	void setresistancepower(float val);
	int gettirednessfixpower();
	void settirednessfixpower(unsigned int val);
	int getattack();
	void setattack(unsigned int val);
	int gettypeofconsumable();
	void settypeofconsumable(unsigned int val);
	float consumable_showpower();

	consumable(std::string cname, float cpower, unsigned int cprice, unsigned int clvlreq, unsigned int ctypeofconsumable, unsigned int ctier) {
		name = cname;
...
struct single_statupdate : public effect { // base struct for stat related effects
	virtual void effectfunc(livingentity* user) { std::cout << "exampleeffect"; };
	virtual void effectcancel(livingentity* user) { std::cout << "exampleeffectcancel"; };

	single_statupdate(unsigned int nexpiretime, float npower, livingentity* user) {
...
protected:
	...
	int basehp;
	int hp;
	unsigned int accuracy = 85;
	unsigned int basetiredness;
	unsigned int tiredness; //less value = more tired
...
public:
	...
	int gettiredness();
	void settiredness(int val);
	void addtiredness(int val);
	int getendurance();
	void setendurance(int val);
	void addendurance(int val);
	int getbasetiredness();
	void setbasetiredness(int val);
	float getdistance();
	...
	float getresistance();
	void setresistance(float val);
	void addresistance(float val);
	float getmobility();
	void setmobility(float val);
	void addmobility(float val);
...
public:
	...
	int getlvlpts();
	void setlvlpts(int val);
	void addlvlpts(int val);
	int getggol();
	void setggol(int val);
	void addggol(int val);
	void cleanup();
	bool checklvl(bool savetype);
	void playerload(std::string lname, unsigned int llvl, unsigned int llvlpts, unsigned int gggol, std::string lidwep, std::string lidarm, std::string lidhelm, std::string liditem);
...
private:
	...
	unsigned int lvlptsvalue;
	unsigned int ggolvalue;
...
public:
	...
	enemy(std::string ename, unsigned int elvl, unsigned int eatk, unsigned int ehp, unsigned int eptval, unsigned int eggolval, std::string eidwep, std::string eidarm, std::string eidhelm, std::string eiditem) {
		name = ename;
		haveattacked = "";
		haveused = "";
		havemoved = "";
...
///////////////////EFFECTS:
struct hardness : public single_statupdate {
	void effectfunc(livingentity* user);
	void effectcancel(livingentity* user);
};
struct fracture : public single_statupdate {
	void effectfunc(livingentity* user);
	void effectcancel(livingentity* user);
};

struct adrenaline : public single_statupdate {
	void effectfunc(livingentity* user);
	void effectcancel(livingentity* user);
};
struct fatigue : public single_statupdate {
	void effectfunc(livingentity* user);
	void effectcancel(livingentity* user);
};

struct focus : public single_statupdate {
	void effectfunc(livingentity* user);
	void effectcancel(livingentity* user);
};
struct confusion : public single_statupdate {
	void effectfunc(livingentity* user);
	void effectcancel(livingentity* user);
};
#endif