...
std::string typetranslator(unsigned int category, unsigned int typeofequipment) {
...
void item::setprice(unsigned int newprice) {
	price = newprice;
}
int item::getlvlreq() {
	return lvlreq;
}
void item::setlvlreq(unsigned int newlvlreq) {
	lvlreq = newlvlreq;
}
...
void weapon::setvalueattack(unsigned int newattack) {
	valueattack = newattack;
}
int weapon::getdirectionattack(unsigned int attacktype) {
	switch (attacktype) {
	case side_attack:
		return sideattack;
		break;
...
int weapon::getdirectiontiring(unsigned int attacktype) {
	switch (attacktype) {
	case side_attack:
...
void weapon::setdirectionattack(unsigned int newattack, unsigned int weapontype) {
	switch (weapontype) {
	case sword_weapon:
...
void consumable::sethealpower(unsigned int newhealpower) {
...
void consumable::settirednessfixpower(unsigned int newtirednessfixpower) {
	tirednessfixpower = newtirednessfixpower;
}
int consumable::getattack() {
	return attack;
}
void consumable::setattack(unsigned int newattack) {
	attack = newattack;
}
int consumable::gettypeofconsumable() {
	return typeofconsumable;
}
void consumable::settypeofconsumable(unsigned int newtypeofconsumable) {
...
int livingentity::getaccuracy() {
	return accuracy;
}
void livingentity::setaccuracy(int val) {
	accuracy = val;
}
void livingentity::addaccuracy(int val) {
	accuracy += val;
}
...
void livingentity::addtiredness(int tirednesss) {
	tiredness = std::clamp(tiredness + tirednesss, (unsigned int)0, basetiredness);
}
...
void player::playerload(std::string lname, unsigned int llvl, unsigned int llvlpts, unsigned int gggol, std::string lidwep, std::string lidarm, std::string lidhelm, std::string liditem) {
	name = lname;
	lvl = llvl;
	lvlpts = llvlpts;
	checklvl(true);
...
// hardness
void hardness::effectfunc(livingentity* user) {
	std::cout << "User got additional resistance";
	user->addresistance(power);
}
void hardness::effectcancel(livingentity* user) {
	user->addresistance(-1 * power);
	std::cout << "User lost additional resistance";
}

//fracture
void fracture::effectfunc(livingentity* user) {
	std::cout << "Victim got fracture";
	user->addresistance(-1 * power);
}
void fracture::effectcancel(livingentity* user) {
	user->addresistance(power);
	std::cout << "Victim recovered from fracture";
}

//adrenaline
void adrenaline::effectfunc(livingentity* user) {
	std::cout << "User got additional power within his arms";
	user->addatk(power);
}
void adrenaline::effectcancel(livingentity* user) {
	user->addatk(-1 * power);
	std::cout << "User lost additional power";
}

//fatigue
void fatigue::effectfunc(livingentity* user) {
	std::cout << "Victim lose power in his muscles";
	user->addatk(-1 * power);
}
void fatigue::effectcancel(livingentity* user) {
	user->addatk(power);
	std::cout << "Victim recovered from fatigue";
}

void focus::effectfunc(livingentity* user) {
	std::cout << "User focused even more on his task";
	user->addaccuracy(power);
}
void focus::effectcancel(livingentity* user) {
	user->addaccuracy(-1 * power);
	std::cout << "User relaxed";
}

void confusion::effectfunc(livingentity* user) {
	std::cout << "Victim got confused";
	user->addaccuracy(power);
}
void confusion::effectcancel(livingentity* user) {
	user->addaccuracy(-1 * power);
	std::cout << "Victim lrecovered from confusion";
}